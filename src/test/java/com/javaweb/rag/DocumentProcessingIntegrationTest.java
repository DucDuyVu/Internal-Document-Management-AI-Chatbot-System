...
class DocumentProcessingIntegrationTest {
    ...
    void process_shouldParseChunkEmbedAndSave_thenNotDuplicateOnRetry() throws InterruptedException {
        ...
        DocumentEntity afterFirstRun = documentRepository.findById(testDocumentId).orElseThrow();
        assertEquals(DocumentStatus.COMPLETED, afterFirstRun.getStatus(),
                "Document phải chuyển sang COMPLETED sau khi xử lý xong");

        List<DocumentChunkEntity> chunksAfterFirstRun = documentChunkRepository.findByDocumentId(testDocumentId);
        ...
        for (DocumentChunkEntity chunk : chunksAfterFirstRun) {
        ...
        List<DocumentChunkEntity> chunksAfterRetry = documentChunkRepository.findByDocumentId(testDocumentId);
    ...
    private void waitUntilFinished(Long documentId) throws InterruptedException {
        for (int i = 0; i < TIMEOUT_SECONDS; i++) {
            DocumentEntity current = documentRepository.findById(documentId).orElseThrow();
