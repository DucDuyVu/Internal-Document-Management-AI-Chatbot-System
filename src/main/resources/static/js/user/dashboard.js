    ...
    if (btn) {
      btn.addEventListener("click", async () => {
        if (typeof logout !== "undefined") {
          ...
          console.error(
            "Hàm logout() không tồn tại, kiểm tra lại auth.js đã load chưa",
          );
        }
      });
    }
...
async function loadHomeData() {
  try {
    ...
    document.getElementById("statChatCount").textContent =
      stats.chatSessionCount || 0;
    document.getElementById("statViewCount").textContent = stats.viewCount || 0;
    document.getElementById("statSearchCount").textContent =
      stats.searchCount || 0;
...
async function loadUserActivities() {
  try {
    ...
    if (!activities || activities.length === 0) {
      list.innerHTML =
        '<li class="activity-item"><div class="activity-dot indigo"></div><div class="activity-info"><p>Chưa có hoạt động nào</p><span>—</span></div></li>';
      return;
    }
...
async function loadRecentDocuments() {
  try {
    ...
    const docs = await apiRequest(
      "/api/user/documents?page=1&size=6&sort=newest",
    );
    ...
    if (!docs || docs.length === 0) {
      grid.innerHTML =
        '<div class="empty-state"><span>📁</span><p>Chưa có tài liệu nào</p></div>';
      return;
    }
...
async function loadUserDocuments() {
  try {
    ...
    const response = await apiRequest(
      `/api/user/documents?page=${UserState.documents.page}&size=${UserState.documents.pageSize}`,
    );
...
function renderUserDocuments() {
  ...
  if (gridView && UserState.documents.view === "grid") {
    if (!docs || docs.length === 0) {
      gridView.innerHTML =
        '<div class="empty-state"><span>📁</span><p>Không tìm thấy tài liệu</p></div>';
    } else {
      gridView.innerHTML = docs
        .map(
          (doc) => `
                <div class="doc-card" onclick="viewUserDocument(${doc.id})">
                    <div class="doc-card-icon">${typeof getFileIcon !== "undefined" ? getFileIcon(doc.fileType) : "📄"}</div>
  ...
  if (listBody && UserState.documents.view === "list") {
    if (!docs || docs.length === 0) {
      listBody.innerHTML =
        '<tr><td colspan="6" class="empty-state"><span>📁</span>Không tìm thấy tài liệu</td></tr>';
    } else {
      listBody.innerHTML = docs
        .map(
          (doc) => `
                <tr>
                    <td>
  ...
  if (typeof updatePagination !== "undefined") {
    updatePagination(
      "docPagination",
      UserState.documents.page,
      Math.ceil(UserState.documents.total / UserState.documents.pageSize),
    );
  }
}
...
function filterUserDocuments() {
  const searchTerm =
    document.getElementById("docSearchInput")?.value?.toLowerCase() || "";
  const typeFilter = document.getElementById("docTypeFilter")?.value || "";
  const sortFilter =
    document.getElementById("docSortFilter")?.value || "newest";

  UserState.documents.filtered = UserState.documents.data.filter((doc) => {
    const matchesSearch =
      !searchTerm || doc.fileName?.toLowerCase().includes(searchTerm);
  ...
  switch (sortFilter) {
    case "newest":
      UserState.documents.filtered.sort(
    ...
    case "oldest":
      ...
      break;
    case "name":
      UserState.documents.filtered.sort((a, b) =>
...
function setDocView(view) {
  ...
  document.getElementById("docGridView").style.display =
    view === "grid" ? "grid" : "none";
  document.getElementById("docListView").style.display =
...
async function loadChatSessions() {
  try {
    ...
    const sessions = await apiRequest("/api/chat/sessions"); // đổi path
...
function renderChatSessionList() {
  ...
  if (!sessions || sessions.length === 0) {
    container.innerHTML =
      '<div class="chat-session-empty"><span>💬</span><br>Chưa có hội thoại nào</div>';
    return;
  }
...
async function openChatSession(sessionId) {
  try {
    ...
    // API thật trả về mảng ChatMessageHistoryResponse, không phải object session
    ...
    if (active) active.style.display = "flex";
    if (title)
      title.textContent = (session && session.title) || "Cuộc hội thoại";
    if (meta) meta.textContent = `${messages.length} tin nhắn`;
...
function renderChatMessages() {
  ...
  if (!messages || messages.length === 0) {
    container.innerHTML =
      '<div style="text-align:center;padding:40px;color:#9ca3af;">Bắt đầu cuộc trò chuyện với AI</div>';
    return;
  }
...
async function sendChatMessage() {
  ...
  try {
    ...
    // Tạo session nếu chưa có — đổi path đúng
    ...
    // Gửi câu hỏi — đúng endpoint /api/chat/ask, đúng body {sessionId, question}
...
async function submitRenameChat() {
  ...
  try {
    ...
    await apiRequest(
      `/api/user/chat-sessions/${UserState.chat.currentSessionId}`,
      {
        method: "PUT",
    ...
    // Update session list
    const session = UserState.chat.sessions.find(
      (s) => s.id === UserState.chat.currentSessionId,
    );
...
function deleteChatSession() {
  ...
  if (
    ...
    !confirmDelete("Bạn có chắc chắn muốn xoá hội thoại này?")
  ) {
    return;
  }
  ...
  if (
    typeof confirmDelete === "undefined" &&
    !confirm("Bạn có chắc chắn muốn xoá hội thoại này?")
  ) {
    return;
  }
...
async function executeDeleteChatSession() {
  try {
    ...
    await apiRequest(`/api/chat/sessions/${UserState.chat.currentSessionId}`, {
      method: "DELETE",
    });
  ...
  try {
    ...
    if (avatarEl) {
      if (profile.avatarUrl) {
        ...
        avatarEl.textContent = (profile.fullName || "U")
          .charAt(0)
          .toUpperCase();
      }
    }
    ...
    if (viewUsername)
      viewUsername.textContent = profile.username || profile.userName || "—";
  ...
  try {
    ...
    if (avatarInput && avatarInput.files.length > 0) {
      ...
      const uploadRes = await apiRequest(
        ...
        true,
      ); // Use raw fetch wrapper if possible, or ensure apiRequest doesn't override Content-Type
...
function changePage(page) {
  if (page < 1) return;
  if (
    page > Math.ceil(UserState.documents.total / UserState.documents.pageSize)
  )
    return;
